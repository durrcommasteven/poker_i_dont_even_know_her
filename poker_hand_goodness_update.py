...
def data_label(n, vocal = False):
	...
	flop_turn_river = lambda : random.choice([0,0,0,0,3,3,4,4,5,5])
	...
	for hand in range(n):
		...
		label = 1-winning_hand(hole1, hole2, community)
...
def test_convergence(reps):
	"""Takes reps as an input
	ie the numer of runs to do
	eg: reps = [10, 100, 1000]

	plots the obtained means
	"""
	...
	for r in reps:
		print('current repetition number: ',r)
...
#I want the median to be equal to zero. I update this from 
#reduced mean squared to reduced mean abs
#mean_loss = 0.5*(tf.reduce_mean(G1)**2 + tf.reduce_mean(G2)**2)
mean_loss = 0.5*(tf.abs(tf.reduce_mean(G1)) + tf.abs(tf.reduce_mean(G2)))
...
if eval_on_card:
	with tf.Session() as sess:
		...
		for i in range(len(holes)//2):
			c1, c2 = holes[2*i]
			c3, c4 = holes[2*i +1]
			v1, v2 = vectorize_data([c1, c2], [c3, c4], ['3c', '5s', 'Td'], normalize = False)
		...
		v1, v2 = vectorize_data(['Ah', 'Jc'], ['Ts', 'Js'], ['3c', '5s', 'Td'], normalize = False)
		g1, g2 = sess.run((G1, G2), feed_dict = {x1_ : [v1], x2_ : [v2]})
...
if extract_weights:
	...
	with tf.Session() as sess:
		...
		names = ['kernel_0.npy', 'bias_0.npy','kernel_1.npy', 'bias_1.npy', 'kernel_2.npy', 'bias_2.npy']
...
with tf.Session() as sess:
	...
	for r in range(reps):
		...
		if r%check_every==0:
			...
			save_path = saver.save(sess, "/updated_tmp_goodness_hidden_1/model_rep_"+str(r)+".ckpt")
			save_path = saver.save(sess, "/updated_tmp_goodness_hidden_1/model_most_recent.ckpt")

			if True:
				#save every 100
				np.save("prediction_losses_1.npy", np.array(pred_losses))
				np.save("mean_losses_1.npy", np.array(mean_losses))
